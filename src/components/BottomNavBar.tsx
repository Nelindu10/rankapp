...
export const BottomNavBar: React.FC<BottomNavBarProps> = ({ activeTab, setActiveTab }) => {
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-5 pt-2 bg-[#201f21]/80 backdrop-blur-xl border-t border-[#4cd7f6]/20 shadow-[0_-4px_20px_rgba(0,0,0,0.6)]">
      <button
        onClick={() => setActiveTab('dashboard')}
        className={`flex flex-col items-center justify-center px-4 py-1.5 rounded-xl transition-all duration-150 active:scale-90 ${
          activeTab === 'dashboard'
